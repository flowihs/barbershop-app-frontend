...
import type { Category, Provision, ProvisionRequest, CreateProvisionRequest } from "../model/types";

export const provisionService = {
    getAll: () => api.get<ProvisionRequest[]>('/api/provisions').then((res) => res.data),
    getById: (id: number) => api.get<ProvisionRequest>(`/api/provisions/${id}`).then((res) => res.data),
    getFreeSlots: (id: number) => api.get<ProvisionRequest>(`/provisions/free/${id}`).then((res) => res.data),
